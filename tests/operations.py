...
async def filter_message_top_n(message,params,guid):
    ...
    with open(f"{guid}_scored{datetime.now()}.json", 'w') as outf:
    ...
    with open(f"{guid}_sortedscores{datetime.now()}.json", 'w') as outf:
        json.dump(message, outf, indent=2)
    ...
    if num_results > n:
        ...
        with open(f"{guid}_filteredtopnresult{datetime.now()}.json", 'w') as outf:
            json.dump(message, outf, indent=2)
        rmessage, status = await filter_kgraph_orphans(fmessage,params,guid)
        logger.info(f'{guid}: Returning filtered message ({n}).')
        with open(f"{guid}_filteredkgorphan{datetime.now()}.json", 'w') as outf:
